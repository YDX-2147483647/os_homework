...
use crate::{Action, Operator, OperatorRole, ReportMessage};

/// 读者优先方案
///
/// 读者申请时，只要已有其它读者正在读，则它可直接开始操作，不理会写者的请求。
pub fn run_read_preferring(operators: Vec<Operator>, tx: Sender<ReportMessage>) {
...
///
/// 一旦有写者申请，任何新读者都必须先等待。
pub fn run_write_preferring(operators: Vec<Operator>, tx: Sender<ReportMessage>) {
    let access = Arc::new((Mutex::new(true), Condvar::new()));
    let n_readers = Arc::new(Mutex::new(0));
...
pub fn run_unspecified_priority(operators: Vec<Operator>, tx: Sender<ReportMessage>) {
    let access = Arc::new((Mutex::new(true), Condvar::new()));
